...
const server = http.createServer(async (req, res) => {
  ...
  if (path === "/auth/discord/callback") {
    ...
    try {
      ...
      console.log("[AUTH] email_otp présent:", !!emailOtp, "hashed_token présent:", !!hashedToken);

      // f) échange l'OTP contre une vraie session. On verifie avec email + token OTP,
      //    méthode la plus fiable côté serveur.
      let session = {};
      if (emailOtp) {
        const verifyRes = await fetch(SUPABASE_URL + "/auth/v1/verify", {
          method: "POST",
          headers: { apikey: SERVICE_ROLE, "Content-Type": "application/json" },
          body: JSON.stringify({ type: "email", email: fakeEmail, token: emailOtp }),
        });
        const verifyBody = await verifyRes.text();
        console.log("[AUTH] verify(email otp) status:", verifyRes.status, "body:", verifyBody.slice(0, 400));
        try { session = JSON.parse(verifyBody); } catch (e) {}
      }
      // Repli : ancienne méthode hashed_token.
      if (!session.access_token && hashedToken) {
        const vr2 = await fetch(SUPABASE_URL + "/auth/v1/verify", {
          method: "POST",
          headers: { apikey: SERVICE_ROLE, "Content-Type": "application/json" },
          body: JSON.stringify({ type: "magiclink", token: hashedToken }),
        });
        const vb2 = await vr2.text();
        console.log("[AUTH] verify(hashed) status:", vr2.status, "body:", vb2.slice(0, 400));
        try { session = JSON.parse(vb2); } catch (e) {}
      }
      if (!session.access_token) throw new Error("supabase session failed — link:" + linkRes.status);
